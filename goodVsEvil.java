...
public class GoodVsEvil {
    public static String battle(String goodAmounts, String evilAmounts) {
        ...
        int[] goodRace = {1, 2, 3, 3, 4, 10};
        int[] evilRace = {1,2,2,2,3,5,10};


        int goodWorth = 0;
        for (int i=0; i < goodAmountsArr.length; i++){
            goodWorth += Integer.parseInt(goodAmountsArr[i]) * goodRace[i];
        }

        int evilWorth = 0;
        for (int i=0; i < evilAmountsArr.length; i++){
            evilWorth += Integer.parseInt(evilAmountsArr[i]) * evilRace[i];
        }
